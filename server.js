...
app.get('/api/stock/:code', async (req, res) => {
  ...
  for (const suf of suffixes) {
    try {
      ...
      // 比對「現價擷取時間」(meta.regularMarketTime)跟「K線陣列最後一筆時間」是否相差超過
      // 12小時,藉此判斷現在是盤中即時報價還是已收盤資料。這是先前美股指數長時間排查
      // 才確認的問題根源(K線陣列最後一筆若還沒被Yahoo正式收進去,會是還在跳動的即時價,
      // 不是真正收盤價),個股查詢用同一套Yahoo API,理論上會有同樣的狀況。
      ...
      const regularMarketTime = meta?.regularMarketTime || lastArrayTime;
      const hoursDiff = Math.abs(regularMarketTime - lastArrayTime) / 3600;

      let finalClose = closesArr[closesArr.length - 1];
      let isLiveSession = false;
      if (hoursDiff > 12 && meta?.regularMarketPrice != null && isTaiwanTradingHours()) {
        // 盤中即時報價:陣列最後一筆其實是「前一個已收盤交易日」,現價要用meta裡的即時價,
        // 且陣列本身(closes/highs/lows/volumes)不需要額外處理,只有「最新現價」這個顯示
        // 數字要改用meta.regularMarketPrice,技術指標計算仍以陣列已收盤資料為準
        isLiveSession = true;
      }
      ...
      const payload = {
        ...
        liveQuote: isLiveSession ? meta.regularMarketPrice : null, // 若為盤中即時報價,額外提供這個欄位,前端可選擇用來顯示「現價」
        isLiveSession,
      };

      // ---- 跟證交所官方即時報價交叉比對,偵測Yahoo資料異常 ----
      // 主報表(PowerShell版)已經有這層交叉比對機制,曾實際抓到過Yahoo現價跟證交所官方
      // 收盤價落差近10%的案例(2426鼎元:Yahoo顯示99.80,證交所官方收盤價90.8),但網頁版
      // 個股查詢原本完全沒有這層防護,單純把Yahoo的數字原封不動端給使用者,一旦Yahoo資料
      // 異常,使用者看到的漲跌方向可能完全是錯的。這裡比照主報表的做法補上同一套交叉比對:
      // 查詢單一股票時,額外打一次證交所官方即時報價API(不用抓整個市場快照,單股查詢
      // 輕量許多),兩邊落差超過5%就在回傳資料裡標註警示,由前端決定怎麼呈現給使用者
      try {
        const exCh = suf === '.TW' ? 'tse' : 'otc';
        const twseUrl = `http://mis.twse.com.tw/stock/api/getStockInfo.jsp?ex_ch=${exCh}_${code}.tw`;
        const twseRes = await fetch(twseUrl, { headers: { 'User-Agent': 'Mozilla/5.0' } });
        const twseJson = await twseRes.json();
        const twseItem = twseJson?.msgArray?.[0];
        // z=最新成交價,若當日尚無成交(例如剛開盤或停牌)會是'-',退回用y(昨收)概略比對
        const twsePriceRaw = twseItem?.z !== '-' ? twseItem?.z : twseItem?.y;
        const twsePrice = twsePriceRaw != null ? parseFloat(twsePriceRaw) : null;
        const yahooPrice = payload.liveQuote != null ? payload.liveQuote : closesArr[closesArr.length - 1];
        if (twsePrice != null && !isNaN(twsePrice) && yahooPrice != null) {
          const gapPct = Math.abs(yahooPrice - twsePrice) / twsePrice * 100;
          if (gapPct >= 5) {
        ...
        // 證交所比對失敗不影響主要查詢結果,靜默略過即可,不用讓整個查詢因此失敗
      }
