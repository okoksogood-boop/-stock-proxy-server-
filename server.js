...
app.post('/api/insight', async (req, res) => {
  ...
  try {
    const prompt = `你是台股分析助手。以下是某檔股票目前的數據資料:\n\n${context}\n\n請根據以上資料,撰寫一份結構化的分析報告(繁體中文,總長300字以內),分成以下段落,每段開頭用「段落名稱:」標示,段落與段落之間直接用全形分隔符號「▲」連接,絕對不要使用換行符號:\n今日價量表現:說明收盤價、漲跌幅、成交量是否高於/低於平均水平\n技術面觀察:說明均線排列、KD、RSI、MACD等指標透露的訊號,若指標間出現矛盾訊號(例如均線多頭但MACD空頭)請明確指出\n法人與籌碼動向:說明三大法人買賣超方向,若有連續多日趨勢請一併說明\n估值與基本面:說明本益比/股價淨值比/殖利率反映的估值高低,以及營收年增率顯示的成長動能\n市場關注:若有提供近期新聞或重大訊息,摘要其重點與可能影響;若無相關資料則說明目前無重大消息面訊息\n\n語氣需像專業分析師一樣客觀中立,不做任何確定性預測,不使用「保證上漲」「一定要買」「現在就賣」等武斷字眼,所有評論僅為資料客觀解讀。請勿編造資料中沒有提到的數字或消息。\n\n請務必只回傳單行JSON物件,絕對不要在JSON字串值裡包含任何換行符號,格式如下,不要有其他文字說明:\n{"analysis":"完整分析內容(段落間用▲分隔,不含換行)","sentiment":"偏多/偏空/中性"}\n(sentiment欄位請根據分析內容整體語氣傾向填寫,須與analysis內容一致)`;
    const url = 'https://api.groq.com/openai/v1/chat/completions';
    const r = await fetch(url, {
      method: 'POST',
      headers: {
