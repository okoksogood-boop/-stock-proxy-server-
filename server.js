...
app.get('/api/revenue/:code', async (req, res) => {
  ...
  try {
    ...
    const yoy = yoyRaw && /^-?[\d.]+$/.test(yoyRaw) ? parseFloat(yoyRaw) : null;
    const mom = momRaw && /^-?[\d.]+$/.test(momRaw) ? parseFloat(momRaw) : null;
...
app.post('/api/insight', async (req, res) => {
  ...
  try {
    const prompt = `你是台股分析助手。以下是某檔股票目前的數據資料:\n\n${context}\n\n請根據以上資料,用一段話(100字以內)綜合分析這檔股票目前技術面、籌碼面、消息面的重點,語氣像專業分析師一樣客觀中立。請勿給出「保證上漲」「一定要買」「現在就賣」這類武斷確定性的投資建議,只做客觀資料解讀。\n\n請務必只回傳JSON物件,格式如下,不要有其他文字說明:\n{"analysis":"分析內容本文","sentiment":"偏多/偏空/中性"}\n(sentiment欄位請根據你分析內容的整體語氣傾向填寫,須與analysis內容一致)`;
    const url = 'https://api.groq.com/openai/v1/chat/completions';
    const r = await fetch(url, {
      method: 'POST',
      headers: {
