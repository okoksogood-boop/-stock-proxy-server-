...
app.get('/api/stock/:code', async (req, res) => {
  ...
  for (const suf of suffixes) {
    try {
      ...
      let timestamps = validIdx.map((i) => result.timestamp[i]);
      let closesArr = validIdx.map((i) => quote.close[i]);
      let validIdxUsed = validIdx;
      const meta = result.meta;
      const todayStr = toTaipeiDateStr(Date.now() / 1000);

      // ---- 幽靈K棒偵測與移除 ----
      // 【問題】Yahoo會在「交易時段還沒開始」的時候,就先在K線陣列尾端放一根當天的K棒,
      // 收盤價直接沿用上一個交易日。非交易日整天都是這樣,交易日則是開盤前(09:00之前)會這樣。
      // 【影響】漲跌幅是拿 closes[n-1] 減 closes[n-2] 算的,兩者相同就變成0%;
      // 而且均線、前高、量比、K線型態全都會被這根假K棒拉歪,不只是漲跌幅一欄。
      // 【判斷依據】不是看成交量(實測那根棒子是有少量成交量的,用量判斷會漏掉),
      // 而是看「這根K棒代表的交易時段有沒有真的開始過」——hasTaiwanMarketOpenedToday()
      // 同時涵蓋週末與平日開盤前兩種情況。
      // 【限制】這裡只判斷週末與開盤前,沒有國定假日清單,所以春節等連假期間仍可能殘留;
      // 主報表(PowerShell版)有完整的假日清單可以處理得更精確。
      if (timestamps.length >= 3) {
        const lastBarIsToday = toTaipeiDateStr(timestamps[timestamps.length - 1]) === todayStr;
        if (lastBarIsToday && !hasTaiwanMarketOpenedToday()) {
          validIdxUsed = validIdx.slice(0, -1);
          timestamps = timestamps.slice(0, -1);
          closesArr = closesArr.slice(0, -1);
        }
      }
      ...
      const payload = {
        ...
        opens: validIdxUsed.map((i) => quote.open[i]),
        closes: closesArr,
        highs: validIdxUsed.map((i) => quote.high[i]),
        lows: validIdxUsed.map((i) => quote.low[i]),
        volumes: validIdxUsed.map((i) => quote.volume[i]),
        adjCloses: adjCloseArr ? validIdxUsed.map((i) => adjCloseArr[i]) : null, // 供前端計算技術指標用,避免除權息造成假訊號
        timestamps,
        insufficientData: validIdxUsed.length < 20,
        dataPoints: validIdxUsed.length,
      ...
      try {
        if (suf === '.TW') {
          ...
          // 官方當日漲跌:用來確認「漲跌幅算出0%」到底是真平盤還是資料有問題。
          // Yahoo偶爾會給出與前一日相同的收盤價,光看數字無從分辨;證交所說漲跌是0就是真平盤。
          // 與主報表 2026-09-24-A 的做法一致
          const officialChange = dayAllItem?.Change != null && dayAllItem.Change !== '' ? parseFloat(dayAllItem.Change) : null;
          if (officialChange != null && !isNaN(officialChange)) {
            payload.officialChange = officialChange;
          }
...
// ---------- 共用的 Groq 呼叫 + 重試機制 ----------
// 原本每個AI端點都是「打一次,失敗就把原始錯誤JSON丟給使用者看」。實務上Groq會偶發回覆
// json_validate_failed(400):這是推理模型在 response_format: json_object 模式下,內部思考
// 過程偶爾沒有正確收尾成合法JSON造成的,跟金鑰、額度、參數都無關,重試一次通常就成功了。
// 主報表(PowerShell版)早就有重試,網頁版一直沒有,所以同一個偶發問題在網頁版會直接變成
// 使用者眼前的一長串英文錯誤訊息。這裡補上同一套重試,並多做一件事:
// 最後一次嘗試時拿掉 response_format,改用純提示詞要求輸出JSON、由我們自己剝除```json```圍欄
// 後解析——因為 json_validate_failed 正是這個參數本身引起的,拿掉它才是真正對症下藥的退路。
function sleepMs(ms) { return new Promise((resolve) => setTimeout(resolve, ms)); }

async function callGroqWithRetry(bodyObj, label, maxRetries = 3) {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    ...
    const body = { ...bodyObj };
    // 最後一次嘗試:拿掉造成 json_validate_failed 的 response_format 參數再試一次
    if (isLastAttempt && body.response_format) delete body.response_format;
    try {
      ...
      lastJson = json;
      lastStatus = r.status;
      // 金鑰無效:重試再多次也不會成功,直接回報讓使用者去換金鑰
      if (isInvalidKeyResponse(json)) return { ok: false, invalidKey: true, status: r.status, json };
      const raw = JSON.stringify(json);
      // 當日額度用完:同樣重試無益
      if (/tokens per day|TPD/i.test(raw)) return { ok: false, quotaExhausted: true, status: r.status, json };

      console.error(`Groq ${label} 第${attempt}/${maxRetries}次失敗 (HTTP ${r.status}):`, raw.slice(0, 500));
      if (!isLastAttempt) {
        const isRateLimit = r.status === 429 || /rate_limit/i.test(raw);
        await sleepMs(isRateLimit ? 3000 : 800);
      }
    } catch (e) {
      console.error(`Groq ${label} 第${attempt}/${maxRetries}次例外:`, e.message);
      lastJson = { error: { message: e.message } };
      if (!isLastAttempt) await sleepMs(800);
    }
  }
  return { ok: false, status: lastStatus, json: lastJson };
}

// 把失敗結果轉成一句使用者看得懂的中文,而不是把原始英文JSON整段丟到畫面上
function describeGroqFailure(result) {
  if (result.invalidKey) return '⚠️ AI金鑰已失效,請前往 Render 後台的 Environment 設定更新 GROQ_API_KEY(申請新金鑰:https://console.groq.com/keys)';
  if (result.quotaExhausted) return '⚠️ 今日AI額度已用完,請明日再試(額度每日重置)';
  return '⚠️ AI分析暫時無法取得(AI服務回應異常,已自動重試多次仍未成功),請稍後再查詢一次。其餘技術面與籌碼面數據不受影響,仍可正常參考。';
}
...
app.post('/api/ask', async (req, res) => {
  ...
  try {
    ...
    if (!result.ok) {
      return res.json({ answer: describeGroqFailure(result) });
    }
...
async function getAnnouncementSentiment(items) {
  ...
  try {
    ...
    }, 'getAnnouncementSentiment');
    if (!result.ok) return {};
    let text = result.content.replace(/^```json\s*/, '').replace(/```\s*$/, '').trim();
    let parsed;
    try {
      parsed = JSON.parse(text);
    } catch (parseErr) {
      // 最後一次嘗試會拿掉response_format,模型可能在JSON前後多寫文字,抓出大括號區塊再試
...
app.post('/api/insight', async (req, res) => {
  ...
  try {
    ...
    if (!result.ok) {
      return res.json({ insight: null, debug: describeGroqFailure(result) });
    }
    ...
    try {
      ...
      // 最後一次嘗試會拿掉response_format,模型有可能在JSON前後多寫幾句話。
      // 這裡退而求其次,從整段文字裡抓出第一個完整的大括號區塊再解析一次
      const m = text.match(/\{[\s\S]*\}/);
      if (m) {
        try { parsed = JSON.parse(m[0]); } catch (e2) { parsed = null; }
      }
      if (!parsed) {
        console.error('Groq /api/insight JSON解析失敗,原始內容:', text.slice(0, 500));
        return res.json({ insight: null, debug: '⚠️ AI回應格式異常無法解析,請稍後再查詢一次。其餘技術面與籌碼面數據不受影響。' });
      }
    }
  ...
  for (const suf of suffixes) {
    try {
      ...
      const points = result.timestamp
        .map((t, i) => ({ time: t, price: closes[i], volume: volumes[i] != null ? volumes[i] : 0 }))
        .filter((p) => p.price != null);
      if (points.length === 0) continue;

      // ---- 這份資料到底是哪一天的?----
      // range=1d 拿到的是「最近一個有交易的日期」的分時資料,不保證是今天。週末、國定假日、
      // 或盤前時段查詢時,Yahoo回的其實是上一個交易日的分時走勢。如果不標示清楚,使用者會
      // 以為看到的是今天的走勢——這跟2426那次「收盤後查詢卻拿到前一日數值」是完全同一類問題,
      // 所以這裡一律回報資料的實際交易日期,由前端誠實標示出來
      const taipeiDate = (unixSec) => new Intl.DateTimeFormat('en-CA', { timeZone: 'Asia/Taipei' }).format(new Date(unixSec * 1000));
      const sessionDate = taipeiDate(points[0].time);
      const todayStr = taipeiDate(Date.now() / 1000);

      return res.json({
        code,
        points,
        sessionDate,                        // 這份分時資料實際對應的交易日(台北時區 YYYY-MM-DD)
        isToday: sessionDate === todayStr,  // 是否就是今天
        // 昨收參考價:分時圖的慣例是以昨收畫一條水平基準線,漲跌用它當分界,沒有這條線
        // 只看絕對價格很難判斷當下是漲是跌
        previousClose: result.meta?.chartPreviousClose != null ? result.meta.chartPreviousClose : null,
        isLiveSession: isTaiwanTradingHours() && sessionDate === todayStr,
      });
    } catch (e) {
      continue;
    }
