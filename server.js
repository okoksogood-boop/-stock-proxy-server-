...
async function getAnnouncementSentiment(items) {
  ...
  try {
    const itemsForPrompt = items.map((it, i) => ({ id: i, subject: it.subject }));
    const prompt = `你是台股分析助手。以下是股票重大訊息公告主旨清單,請針對每一則判斷對股價可能是「利多」「利空」或「中性」,並給一句話簡短理由(15字以內)。請務必只回傳JSON陣列,格式如下,不要有其他文字說明:\n[{"id":0,"sentiment":"利多/利空/中性","reason":"簡短理由"}]\n\n公告清單:\n${JSON.stringify(itemsForPrompt)}`;
